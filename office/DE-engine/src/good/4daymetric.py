...
NODES = [
    {"name": "core_node", "ip": "192.168.88.38:9100"},  # Replaced space with underscore
...
for node in NODES:
    node_name = node['name']
    for metric in ['cpu', 'memory', 'tx', 'rx', 'latency', 'cost']:
...
async def process_metrics(json_gnb_ueget, node_metrics, now):
    # Enhanced RAN metrics with UE IDs and detailed structure
    ran_metrics = {'time': now.timestamp()}

    if len(json_gnb_ueget['ue_list']) == 0:
        ran_metrics.setdefault('ue_list', [])
    else:
        # Add UE IDs and maintain detailed structure
        ue_list_with_ids = []
        num_ues = len(json_gnb_ueget['ue_list'])
        for ue_ix in range(num_ues):
            ue_data = json_gnb_ueget['ue_list'][ue_ix].copy()
            ue_data['ue_id'] = str(num_ues - ue_ix)  # Add calculated UE ID
            ue_list_with_ids.append(ue_data)

        ran_metrics.setdefault('ue_list', ue_list_with_ids)
    ...
    # Print the metrics like the original
    ...
    if ue_list:
        # Sort by ue_id ascending
        ue_list_sorted = sorted(ue_list, key=lambda x: int(x['ue_id']))
        for i, ue in enumerate(ue_list_sorted):
            if i >= MAX_UES:
                print(f"Warning: More than {MAX_UES} UEs detected. Skipping extra UEs.")
                break
            cells = ue.get('cells', [{}])[0]
            row[f'ue{i+1}_cqi'] = cells.get('cqi', 0.0)
            row[f'ue{i+1}_dl_bitrate'] = cells.get('dl_bitrate', 0.0)
            row[f'ue{i+1}_dl_mcs'] = cells.get('dl_mcs', 0.0)
            row[f'ue{i+1}_epre'] = cells.get('epre', 0.0)
            row[f'ue{i+1}_pusch_snr'] = cells.get('pusch_snr', 0.0)
            row[f'ue{i+1}_ul_bitrate'] = cells.get('ul_bitrate', 0.0)
            row[f'ue{i+1}_ul_mcs'] = cells.get('ul_mcs', 0.0)
            row[f'ue{i+1}_ul_path_loss'] = cells.get('ul_path_loss', 0.0)

    # Append to all CSV (check header at start only, but since append, assume handled outside)
...
async def main():
    print("Starting combined metrics collection (Amarisoft + Prometheus) for 4 days")
    ...
    # Prepare all CSV header if not exists
    ...
    duration = 4 * 24 * 3600  # 4 days in seconds
    ...
    while time.time() - start_time < duration:
        ...
        # Process metrics: print and save to CSVs
