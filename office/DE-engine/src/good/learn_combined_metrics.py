...
def expose_prometheus_metrics(requests_sent, json_gnb_ueget):
    ...
    for idx, ue in enumerate(json_gnb_ueget['ue_list']):
        ue_id = str(num_ues - idx)
...
def save_csv_and_print(now, json_gnb_ueget, node_metrics):
    ...
    headers = [
        ...
        *[f"{node}_{metric}" for node in ["core_node","mec_node","kube_app_server"] for metric in ["cpu","memory","tx","rx","latency","cost"]],
        "ue_count",
        "ue_ids",
        "avg_cqi","avg_dl_bitrate","avg_ul_bitrate","avg_dl_mcs","avg_ul_mcs","avg_epre","avg_pusch_snr","avg_ul_path_loss"
    ]
    ...
    if ue_count > 0:
        avg_cqi = sum([ue['cells'][0]['cqi'] for ue in ue_list])/ue_count
        avg_dl_bitrate = sum([ue['cells'][0]['dl_bitrate'] for ue in ue_list])/ue_count
        avg_ul_bitrate = sum([ue['cells'][0]['ul_bitrate'] for ue in ue_list])/ue_count
        avg_dl_mcs = sum([ue['cells'][0].get('dl_mcs',0) for ue in ue_list])/ue_count
        avg_ul_mcs = sum([ue['cells'][0].get('ul_mcs',0) for ue in ue_list])/ue_count
        avg_epre = sum([ue['cells'][0]['epre'] for ue in ue_list])/ue_count
        avg_pusch_snr = sum([ue['cells'][0]['pusch_snr'] for ue in ue_list])/ue_count
        avg_ul_path_loss = sum([ue['cells'][0].get('ul_path_loss',0) for ue in ue_list])/ue_count
    else:
        avg_cqi = avg_dl_bitrate = avg_ul_bitrate = avg_dl_mcs = avg_ul_mcs = avg_epre = avg_pusch_snr = avg_ul_path_loss = 0
    ...
    row.extend([ue_count, ",".join(ue_ids), avg_cqi, avg_dl_bitrate, avg_ul_bitrate, avg_dl_mcs, avg_ul_mcs, avg_epre, avg_pusch_snr, avg_ul_path_loss])
    ...
    print(f"Avg CQI={avg_cqi:.2f}, Avg DL={avg_dl_bitrate:.2f} bps, Avg UL={avg_ul_bitrate:.2f} bps, Avg DL MCS={avg_dl_mcs:.2f}, Avg UL MCS={avg_ul_mcs:.2f}")
