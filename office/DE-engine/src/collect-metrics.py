...
PROMETHEUS_SERVER = 80  # Default Prometheus port
ALTERNATIVE_PORT = 8080  # Fallback port if 80 is in use
...
def signal_handler(sig, frame, loop):
    ...
    if prom_server:
        # Close the Prometheus server (not directly supported by prometheus_client, so we rely on process termination)
...
async def main():
    ...
    # Try starting Prometheus server on default port, fallback to alternative
    port = PROMETHEUS_SERVER
    if not start_prometheus_server(port):
        print(f"Trying alternative port {ALTERNATIVE_PORT}")
        port = ALTERNATIVE_PORT
        if not start_prometheus_server(port):
            print(f"Failed to start Prometheus server on port {ALTERNATIVE_PORT}. Exiting.")
            sys.exit(1)
