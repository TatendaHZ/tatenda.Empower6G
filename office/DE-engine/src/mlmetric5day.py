...
ALL_CSV_FILE = "seven_days_metrics.csv"
MAX_UES = 3
...
UE_METRICS = [
    'cqi','dl_bitrate','dl_mcs','epre','pusch_snr','ul_bitrate','ul_mcs','ul_path_loss',
    'dl_tx','ul_tx','dl_err','ul_err','dl_retx','ul_retx','ul_phr','ul_rank','ul_n_layer',
    'turbo_decoder_min','turbo_decoder_avg','turbo_decoder_max'
]
...
async def process_metrics(json_gnb_ueget, node_metrics, now):
    ...
    for i, ue in enumerate(ue_list):
        ...
        ue_metrics = {metric: cells.get(metric, 0.0) for metric in UE_METRICS}
        ue_metrics['ue_id'] = i+1
        ue_output.append(ue_metrics)
        for k, v in ue_metrics.items():
            if k != 'ue_id':
                row[f'ue{i+1}_{k}'] = v

    # Print full JSON live
    combined_output = {
...
async def main():
    ...
    while time.time() - start_time < duration:
        ...
        print(f"Requesting to API uri: ws://{TARGET_ENB}")
