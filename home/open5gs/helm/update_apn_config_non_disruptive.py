...
try:
    with open("apn_config11.json", "r") as file:
        apn_config = json.load(file)
    apn_template = apn_config.get("apn_template", {})
    ue_ambr_default = apn_config.get("ue_ambr", {})
...
try:
    ...
    for subscriber in subscribers_collection.find():
        ...
        # Use conditional UE-AMBR based on IMSI
        ue_ambr = ue_ambr_default
        if subscriber["imsi"] == "001011234567895":
            ue_ambr = {
                "downlink": {"value": 1, "unit": 3, "string": "1 Mbps"},
                "uplink": {"value": 1, "unit": 3, "string": "1 Mbps"}
            }
        ...
        for slice in subscriber.get("slice", []):
            ...
            for session in slice.get("session", []):
                ...
                if apn_name in apn_template:
                    # Update session AMBR, QoS, and type
    ...
    print("New ue_ambr configuration (default):")
    print(json.dumps(ue_ambr_default, indent=2))
    print("Note: IMSI 001011234567895 uses ue_ambr: 1 Mbps (unit: 3). Updates should not affect active sessions unless re-registration occurs.")
